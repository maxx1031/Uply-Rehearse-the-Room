    ...
    <div style={{ position: "absolute", inset: 0 }}>
      <StageBackdrop>
        <div style={{
          ...
          <ActLabel color="#8881b8">SET YOUR SCENE</ActLabel>
          ...
          <div style={{ color: "#8881b8", fontSize: 14, marginTop: 8, marginBottom: 24 }}>
            ...
            {GOALS.map(g => {
              ...
              return (
                <button key={g.id} onClick={() => setPicked(g.id)} style={{
                  ...
                  border: active ? "1.5px solid #b8acf6" : "1px solid rgba(184,172,246,.18)",
                  color: "#ebe6fb", cursor: "pointer", fontFamily: "inherit",
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, lineHeight: 1.2, color: "#fff" }}>{g.title}</div>
                    <div style={{ fontSize: 12.5, color: "#a89ed4", marginTop: 3 }}>{g.sub}</div>
                    <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: ".22em", color: "#f3d27e", marginTop: 6 }}>{g.scene}</div>
                  </div>
                  <div style={{
                    width: 22, height: 22, borderRadius: "50%",
                    border: active ? "2px solid #b8acf6" : "1.5px solid rgba(184,172,246,.3)",
                    background: active ? "#b8acf6" : "transparent",
                    display: "flex", alignItems: "center", justifyContent: "center",
                  }}>
                    {active && <svg width="11" height="11" viewBox="0 0 14 14"><path d="M2 7 L6 11 L12 3" stroke="#1d1452" strokeWidth="2.4" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                  </div>
                </button>
              );
            })}
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: "#0a0726", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div className="uply-serif uply-fade-up" style={{
          fontSize: 22, fontStyle: "italic", color: "#b8acf6",
        ...
        <div className="uply-fade-up" style={{
          ...
          color: "#8881b8", fontSize: 12, letterSpacing: ".32em", fontWeight: 700,
        }}>
          <div style={{ width: 24, height: 1, background: "#8881b8" }} />
          END OF PROLOGUE
          <div style={{ width: 24, height: 1, background: "#8881b8" }} />
        </div>
      </div>
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
        <div style={{
          ...
          background: "linear-gradient(135deg,#9c8ff0,#5a4ad9)",
          display: "flex", alignItems: "center", justifyContent: "center",
          color: "#fff", fontWeight: 800, fontFamily: "'Playfair Display',serif",
          fontSize: 20, boxShadow: "0 6px 14px rgba(90,74,217,.3)",
        }}>{(user?.name || "U")[0].toUpperCase()}</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#8881b8", letterSpacing: ".22em" }}>WELCOME, ACTOR</div>
          <div className="uply-serif" style={{ fontSize: 20, fontWeight: 700, color: "#1d1452" }}>{user?.name || "Friend"}</div>
        </div>
        {onRestart && (
          <button onClick={onRestart} style={{
            padding: "7px 12px", borderRadius: 9999, border: "1px solid #d8d2c0",
            background: "#fefcf6", color: "#5a4ad9", fontWeight: 700, fontSize: 12,
      ...
      <div style={{
        position: "relative",
        background: "linear-gradient(135deg, #2a1f7a 0%, #5a4ad9 100%)",
        ...
        <ActLabel color="#b8acf6">YOUR FIRST SCENE</ActLabel>
        <div className="uply-serif" style={{ fontSize: 24, fontWeight: 700, marginTop: 8, lineHeight: 1.2 }}>
          {goal.title}
        </div>
        <div style={{ fontSize: 13, color: "#d8d0f9", marginTop: 6 }}>{goal.sub}</div>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} style={{ flex: 1, height: 5, borderRadius: 9999,
                  background: i === 1 ? "#f3d27e" : "rgba(255,255,255,.18)" }} />
              ))}
            </div>
            <div style={{ fontSize: 11, color: "#d8d0f9", fontWeight: 600, letterSpacing: ".18em" }}>
              {goal.scene} · 5 SCENES
            </div>
          </div>
        </div>
        <button style={{
          ...
          background: "#fff", color: "#1d1452", fontWeight: 800, fontSize: 15,
      ...
      <div style={{
        marginTop: 14, background: "#fefcf6", borderRadius: 18, padding: "14px 16px",
        boxShadow: "0 4px 12px rgba(8,4,40,.06)",
        display: "flex", alignItems: "center", gap: 14,
      }}>
        <div style={{ fontSize: 30 }}>🎭</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".22em", color: "#5a4ad9" }}>WARM-UP · 2 MIN</div>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#1d1452", marginTop: 2 }}>Tonight's icebreaker line</div>
        </div>
        <div style={{ color: "#5a4ad9", fontSize: 20 }}>→</div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginTop: 14 }}>
        {[{ n: "1", l: "SCENE DONE" }, { n: "🌙", l: "YOUR ROLE" }, { n: "T·17", l: "YOUR SEAT" }].map((s, i) => (
          <div key={i} style={{
            background: "#fefcf6", borderRadius: 16, padding: "14px 10px", textAlign: "center",
            boxShadow: "0 4px 12px rgba(8,4,40,.06)",
          }}>
            <div className="uply-serif" style={{ fontSize: 22, fontWeight: 700, color: "#1d1452" }}>{s.n}</div>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".18em", color: "#8881b8", marginTop: 4 }}>{s.l}</div>
          </div>
        ))}
      </div>

      <div style={{ marginTop: 18 }}>
        <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".28em", color: "#8881b8", marginBottom: 10, paddingLeft: 4 }}>YOUR CAST</div>
        <div style={{
          background: "#fefcf6", borderRadius: 18, padding: "14px 16px",
          display: "flex", alignItems: "center", gap: 14,
          boxShadow: "0 4px 12px rgba(8,4,40,.06)",
        }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#1d1452" }}>Maya Chen</div>
            <div style={{ fontSize: 12, color: "#5d567f", marginTop: 2 }}>Connected · After Party</div>
          </div>
          <div style={{
            padding: "6px 10px", borderRadius: 9999,
            background: "#ebe6fb", color: "#5a4ad9", fontWeight: 800, fontSize: 10, letterSpacing: ".16em",
          }}>FOLLOWED ✓</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          display: "flex", gap: 6, background: "#1d1452", borderRadius: 9999, padding: 6,
          ...
          {[{ icon: "🎭", label: "Stage", active: true }, { icon: "📜", label: "Scripts" },
            ...
            <div key={i} style={{
              ...
              background: t.active ? "#5a4ad9" : "transparent",
