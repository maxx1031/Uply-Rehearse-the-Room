...
export const LUNA_PERSONA: Persona = {
  ...
  voice: "Warm, curious, easy to talk to. Like a chatty UBC classmate, not a panel speaker.",
  contextLine:
...
export const SELF_INTRO_LESSONS: LessonConfig[] = [
  {
    ...
    tips: ["Name plus school", "Major, role, or current identity", "One natural line Luna can react to"],
    conversationGoal:
      "Naturally learn the user's name, school, and major, role, or current identity. Ask for missing items one at a time, like a real orientation chat.",
  ...
  switch (lesson.id) {
    case "level-1":
      return [
        ...
        "",
        "Facts about yourself you can sprinkle if it fits: the capstone on conversational AI, climbing at The Hive, the fact that Cog Sys mixes CS / psych / linguistics.",
        "",
        "WHERE TO LAND: the user has shared their name and one identity anchor.",
      ].join("\n");
    case "level-2":
      return [
        "OPENING DENSITY: name plus school plus one short phrase about your general field. Two short sentences max. Do not cram in the podcast, photography, or specific projects yet.",
        `Your opening (use this exact line or very close to it): "${lesson.partnerOpening}"`,
...
function makeSelfIntroSystemPrompt(lesson: LessonConfig, memory: IntroMemory): string {
  ...
  return [
    ...
    "- Dump all your background facts in one opening line. Drip them across turns.",
    "- End with coach-style lines like 'let's keep that line for the next round' or 'that version is ready to take into a real coffee chat'.",
    "",
    "CONVERSATION STATE MACHINE",
    "1. OPEN: begin with the exact opening style for this round. Stop and give space.",
    "2. COLLECT TARGET: internally check only this round's target. If it is missing, react to what the user actually said and ask one small question that draws out the missing target.",
    ...
    "",
    "TRUE EARLY EXIT",
    "- Only if the user clearly and explicitly says they have to leave, skip ahead to step 4: give one supportive in-character goodbye and call finish_practice in the same response.",
