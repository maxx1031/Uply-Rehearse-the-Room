...
6. Mission 展示 partner、个人目标、策略和 prompt seed。
...
5. 用户完成或跳过任务。
6. 完成任务后进入 LinkedIn 结果动画。
7. 系统展示 analyzing loading。
8. 系统展示 stage role 结果。
9. 用户回答这个结果是否贴近真实的自己。
10. 用户选择一个小社交目标。
11. slogan 过场后进入 Home。

当前实现状态:

- 已实现 ticket、curtain、after party、task modal、dialogue、LinkedIn result、analyzing、result、reflection、goal、slogan、Home。
...
- onboarding 选择只影响 Mission personal goal 和 practice prompt seed。
...
目标: 在正式练习前，把场景、对话伙伴、用户个人目标、可用策略和 prompt seed 对齐。
...
- `reflectionBucket`: 用户对结果真实感的反馈，`left`、`mid`、`right`。
